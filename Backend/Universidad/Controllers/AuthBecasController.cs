...
{
    [ApiController]
    [Route("api/becas/auth")]
    public class AuthBecasController : ControllerBase
    {
        private readonly IHttpClientFactory _httpClientFactory;
        private readonly AppDbContext _context;
        
        public AuthBecasController(IHttpClientFactory httpClientFactory, AppDbContext context)
        {
        ...
        {
            var ClientId = "TU_CLIENT_ID_BECAS";
            var redirectUri = "http://localhost:5024/api/becas/auth/callback";
        ...
        public async Task<IActionResult> Callback([FromQuery] string code)
        {
            ...
            var ClientId = "TU_CLIENT_ID_BECAS";
            var ClientSecret = "TU_CLIENT_SECRET_BECAS";
            var redirectUri = "http://localhost:5024/api/becas/auth/callback";
            ...
            // Buscar o crear usuario en tu tabla de becas
            // Ejemplo (ajusta según tus entidades):
            /*
            var usuario = await (from u in _context.UsuariosBecas
                                 where u.NombreUsuario == username
                                 select u).FirstOrDefaultAsync();
            
            if (usuario == null)
            {
                usuario = new UsuarioBecas
                {
                ...
                await _context.UsuariosBecas.AddAsync(usuario);
            }
            else
            {
